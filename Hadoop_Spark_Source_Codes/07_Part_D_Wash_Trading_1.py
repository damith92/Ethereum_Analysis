...
class Proj_Part_D_3_1(MRJob):
	...
	def mapper_1(self, _,line):
		...
		try:
			if len(fields) == 7:
				...
				week_val = int(time.strftime("%W", time.gmtime(timef))) #extract week of the year
				year_val = int(time.strftime("%y", time.gmtime(timef))) #extract year
				if value > 0:
					from_key = (from_address, year_val, week_val)
					from_data = (value, 1)
					to_key = (to_address, year_val, week_val)
					to_data = (value, 2)
	...
	def reducer_1(self,word,counts):
		...
		try:
			...
			if sent_value and received_value and sent_value == received_value :
